...
- 首页运行状态总览
  首页直接展示当前是否可用、刷新计划、采集链路和 mitm 状态，能快速判断“现在能不能用”
- 二维码链接推送
...
如果你是第一次在这台机器上跑新版微信，也可以直接打开 [http://127.0.0.1:5000/health](http://127.0.0.1:5000/health)，在页面底部使用“点击确认式模板采集向导”。
...
程序会在你按下鼠标的那一刻按点击位置立即裁图，并把本机模板写到 [cv_templates_local](cv_templates_local)，最后自动刷新上面的模板状态区。
...
- `POST /api/template_capture_click`
  等待一次点击确认并采集当前步骤的本机模板；体检页底部的“点击确认式模板采集向导”就是调用这个接口
- `GET /api/template_capture_preview/<role>`
  查看某张本机模板的最新预览图
